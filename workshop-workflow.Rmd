...
rownames(batches) <- batches$Run_s
...
fourDataset <- AddMetaData(object = fourDataset, metadata = batches)
...
zeisel <- AddMetaData(object = zeisel, metadata = zeisel.meta[,c("Run_s","Owner")])
...
romanov <- ScaleData(romanov, display.progress = F)

marques.data <- as(as.matrix(marques.data), "dgCMatrix")
marques <- CreateSeuratObject(raw.data = marques.data)
marques <- AddMetaData(object = marques, metadata = marques.meta[,c("Run_s","Owner")])
...
marques <- ScaleData(marques, display.progress = F)
